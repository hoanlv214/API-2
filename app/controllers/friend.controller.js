...
Người dùng truyền đúng mã phiên đăng nhập và các 
...
exports.get_user_friends = function (req, res) {
    ...
    if (index <= 0 || index == null || index == undefined || count == null || count <= 0 || count == undefined || token == null || token == "undefined" || token.length == 0 || token == "") {
        Erro.code1004(res);
    }
    else {
            ...
            if (err) {
                ...
                if (userchecktoken.length !== 0) {
                    if (user_id == null || user_id == "undefined" || user_id <= 0) {
                        user_id = userchecktoken[0].user_id;
                    }
                    Friend.CheckAllFriendByIdUserA(user_id, (err, user) => {
                        ...
                        if (err) {
                            ...
                            if (user.length != 0) {
                                ...
                                res.send(JSON.stringify({
                                    message: 'ok',
                                    data: 'không tìm thấy kết quả nào',
                                }))
                            }
                        }
                    })
...
/*
tuan 6: API gửi lời mời kết bạn nhưng bị lỗi
